...
dict = {'itching': 0, 'skin_rash': 0, 'nodal_skin_eruptions': 0, 'continuous_sneezing': 0,
...
def predict_disease_from_symptom(symptom_list):
    ...
    rf = joblib.load('saved_model/RandomForestClassifier.joblib')
...
def main():
    sym = []
    st.header('Disease Prediction Using symptoms :hospital:')
    ...
    st.markdown("Select a symptom from the list and click submit to get predicted Disease as the Output.\n[ NOTE: This app is meant for demo purposes only. Please consult a Doctor if you have any symptoms. ]")
    for i in dict.keys():
        a = st.checkbox(i)
        if a:
            sym.append(i)
    ...
    if st.button('Predict'):
        pred = predict_disease_from_symptom(sym)
        st.success(pred)
